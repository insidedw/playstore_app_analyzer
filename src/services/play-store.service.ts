...
export class PlayStoreService {
  private readonly languages = ['en', 'ar', 'de', 'es', 'fr', 'it', 'ja', 'ko', 'zh-TW', 'zh'];
  
  constructor(private configService: ConfigService) {}

  private async getReviewsForLanguage(page: puppeteer.Page, appId: string, language: string) {
    console.log(`[PlayStoreService] Fetching reviews for language: ${language}`);
    ...
    console.log(`[PlayStoreService] Found ${reviews.length} reviews for ${language}`);
    return reviews;
  }

  async getAppReviews(appId: string) {
    console.log(`[PlayStoreService] Starting to fetch reviews for app: ${appId}`);
    ...
    try {
      ...
      for (const language of this.languages) {
        try {
          const reviews = await this.getReviewsForLanguage(page, appId, language);
          allReviews.push(...reviews.map(review => ({
            ...review,
            language
          })));
        } catch (error) {
          console.error(`[PlayStoreService] Error fetching reviews for ${language}:`, error);
          // Continue with next language even if one fails
          continue;
        }
      }

      console.log(`[PlayStoreService] Total reviews collected across all languages: ${allReviews.length}`);
      return allReviews;
    } catch (error) {
